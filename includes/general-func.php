...
function generateRandomString($length = 5, $case_sensitive = true) {
	if($case_sensitive)
    ...
    for ($i = 0; $i < $length; $i++) {
        // Map byte to character index using modulo
...
/**
 * Get the current active cycle based on current date
 * Returns the cycle where current date falls between start_date and end_date
 */
function getCurrentCycleByDate() {
    try {
        $today = date('Y-m-d');
        
        // Get cycle where current date falls between start and end dates
        ...
        $cycle_stmt = \eBizIndia\PDOConn::query($cycle_sql, [':today' => $today]);
        $current_cycle = $cycle_stmt->fetch(\PDO::FETCH_ASSOC);
        
        return $current_cycle ?: null;
        
    } catch (Exception $e) {
        error_log("Error getting current cycle by date: " . $e->getMessage());
        return null;
    }
}

/**
 * Get the current week number for a given cycle
 * Returns 0 if cycle hasn't started, 1-12 for active weeks, or 13+ if cycle is completed
 */
function getCurrentWeekNumber($cycle) {
    if (!$cycle || !isset($cycle['start_date'])) {
        return 0;
    }
    ...
    try {
        ...
        if ($today < $start_date) {
            ...
            return 13; // Cycle completed (beyond 12 weeks)
        } else {
            $days_passed = $start_date->diff($today)->days;
            return min(floor($days_passed / 7) + 1, 12);
        }
        
    } catch (Exception $e) {
        error_log("Error calculating current week number: " . $e->getMessage());
        return 0;
    }
}
